...
void SPC_DSP::copy_state( unsigned char** io, copy_func_t copy )
{
	...
	for ( i = 0; i < voice_count; i++ )
	{
		...
		// Serialize/deserialize the offset
		{
			uint offset = v->buf_pos - v->buf;
			SPC_COPY(  uint16_t, offset );
			v->buf_pos = v->buf + offset;
		}
		...
		SPC_COPY(  uint8_t, v->volume[0] );
		SPC_COPY(  uint8_t, v->volume[1] );
		SPC_COPY(  uint8_t, v->enabled );
	...
	SPC_COPY( uint16_t, m.counters[0] );
	SPC_COPY( uint16_t, m.counters[1] );
	SPC_COPY( uint16_t, m.counters[2] );
	SPC_COPY( uint16_t, m.counters[3] );
	
	SPC_COPY(  uint8_t, m.new_kon );
	SPC_COPY(  uint8_t, m.t_koff );
	SPC_COPY(  uint8_t, m.surround_threshold );
