auto APU::disassembleInstruction(maybe<uint16> _pc) -> string {
...
auto APU::disassembleContext() -> string {
  string s;
...
auto APU::disassemble(uint16 pc, uint8 prefix, uint8 code) -> string {
  auto byte = [&] {
    return read(pc++);
  };
...
auto APU::disassembleCB(uint16 pc, uint8 prefix, uint8 code) -> string {
  auto byte = [&] {
    return read(pc++);
  };
...
auto APU::disassembleCBd(uint16 pc, uint8 prefix, int8 d, uint8 code) -> string {
  auto displace = [&] {
    return d >= 0 ? string{"+$", hex(d, 2L)} : string{"-$", hex(-d, 2L)};
  };
...
auto APU::disassembleED(uint16 pc, uint8 prefix, uint8 code) -> string {
  auto byte = [&] {
    return read(pc++);
  };
