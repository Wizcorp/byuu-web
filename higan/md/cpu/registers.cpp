template<uint Size> auto CPU::read(DataRegister reg) -> uint32 {
  return clip<Size>(r.d[reg.number]);
}

template<uint Size> auto CPU::write(DataRegister reg, uint32 data) -> void {
  r.d[reg.number] = (r.d[reg.number] & ~mask<Size>()) | (data & mask<Size>());
}

//

template<uint Size> auto CPU::read(AddressRegister reg) -> uint32 {
  return sign<Size>(r.a[reg.number]);
}

template<uint Size> auto CPU::write(AddressRegister reg, uint32 data) -> void {
...
auto CPU::readCCR() -> uint8 {
  return r.c << 0 | r.v << 1 | r.z << 2 | r.n << 3 | r.x << 4;
}

auto CPU::readSR() -> uint16 {
  return readCCR() << 0 | r.i << 8 | r.s << 13 | r.t << 15;
}

auto CPU::writeCCR(uint8 ccr) -> void {
  r.c = ccr.bit(0);
  r.v = ccr.bit(1);
  r.z = ccr.bit(2);
  r.n = ccr.bit(3);
...
auto CPU::writeSR(uint16 sr) -> void {
  writeCCR(sr);
