...
auto CPU::prefetched(EffectiveAddress& ea) -> uint32 {
...
template<uint Size> auto CPU::fetch(EffectiveAddress& ea) -> uint32 {
  if(ea.valid) return ea.address;
  ea.valid = true;
...
template<uint Size, bool hold, bool fast> auto CPU::read(EffectiveAddress& ea) -> uint32 {
  fetch<Size>(ea);
...
template<uint Size, bool hold> auto CPU::write(EffectiveAddress& ea, uint32 data) -> void {
  fetch<Size>(ea);
