auto VDP::VRAM::read(const uint16 address) const -> uint16 {
  if(mode == 0) {
...
auto VDP::VRAM::write(const uint16 address, const uint16 data) -> void {
  if(mode == 0) {
...
auto VDP::VRAM::readByte(const uint17 address) const -> uint8 {
  return read(address >> 1).byte(!address.bit(0));
}

auto VDP::VRAM::writeByte(const uint17 address, const uint8 data) -> void {
  auto word = read(address >> 1);
  word.byte(!address.bit(0)) = data;
  write(address >> 1, word);
}

auto VDP::VSRAM::read(const uint6 address) const -> uint10 {
  if(address >= 40) return 0x0000;
  return memory[address];
}

auto VDP::VSRAM::write(const uint6 address, const uint10 data) -> void {
  if(address >= 40) return;
  memory[address] = data;
}

auto VDP::CRAM::read(const uint6 address) const -> uint9 {
  return memory[address];
}

auto VDP::CRAM::write(const uint6 address, const uint9 data) -> void {
  memory[address] = data;

  //ppcccc -> cccc-pp
  const uint7 offset = (address >> 4 | address << 3) & 0x7b;
